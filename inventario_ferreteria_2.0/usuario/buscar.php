...
include ("../conexion.php");
	...
	$html=" ";
	$registros=$base->query("SELECT SQL_CALC_FOUND_ROWS usu.id_usuario,age.nombre as agencia, concat_ws(' ',emp.nombre,emp.apellido_paterno,emp.apellido_materno) as empleado,usu.tipo_usuario, DATE_FORMAT( usu.registro,  '%d-%m-%Y' ) as registro FROM tbl_usuarios usu inner join tbl_agencias age on usu.id_agencia=age.id_agencia  inner join tbl_empleados  emp on usu.id_empleado=emp.id_empleado where usu.estado='$estado' and age.nombre like '%$busc_empl%' or usu.estado='$estado' and  concat_ws(' ',emp.nombre,emp.apellido_paterno,emp.apellido_materno) like '%$busc_empl%' or usu.estado='$estado' and usu.tipo_usuario like '%$busc_empl%' or usu.estado='$estado' and   DATE_FORMAT( usu.registro,  '%d-%m-%Y' ) like '%$busc_empl%'  order by usu.id_usuario limit $inicio , $regpagina")->fetchAll(PDO::FETCH_OBJ);
...
$html.="</tbody></table>";
 if($Totalregistros>=1):
        $html.="<nav aria-label='Page navigation' class='text-center'>
                    <ul class='pagination'>";
                        if($pagina==1):
