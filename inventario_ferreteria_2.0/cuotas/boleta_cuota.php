...
$registros=$base->query("SELECT cuo.monto,DATE_FORMAT( cuo.fecha,  '%d/%m/%Y' ) AS fecha, cuo.numero FROM tbl_cuotas cuo inner join  pago 
	on cuo.id_pago=pago.id_pago where pago.id_pago='$id'");
...
$pdf->Cell(80,6,'Concepto',1,0,'C',1);
$pdf->Cell(50,6,'Monto',1,1,'C',1);
...
while($dato=$registros->fetch(PDO::FETCH_ASSOC)){
	...
	$i++;
$pdf->Row(array($i,utf8_decode($dato['fecha']) ,utf8_decode("Pago de la cuota Nº ".$dato['numero']),utf8_decode($dato['monto'])));
