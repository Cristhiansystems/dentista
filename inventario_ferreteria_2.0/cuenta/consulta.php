...
include ("../conexion.php");
	...
	$html="";
	if($est=="activo"){
		$registros=$base->query("SELECT cuenta.id_cuenta, nombre, apellido_paterno, apellido_materno, fecha, costo, ci, cuenta.estado From CUENTA INNER JOIN tbl_pacientes on tbl_pacientes.id_paciente=cuenta.id_paciente 
			where tbl_pacientes.codigo_paciente='$ci' and cuenta.estado='$est' AND 0=(select Count(*) as numero from tbl_cuotas WHERE id_cuenta= cuenta.id_cuenta) 
			or tbl_pacientes.ci='$ci' and cuenta.estado='$est' AND 0=(select Count(*) as numero from tbl_cuotas WHERE id_cuenta= cuenta.id_cuenta) 
			or cuenta.fecha between '$feci' and '$fecf' and cuenta.estado='$est' AND 0=(select Count(*) as numero from tbl_cuotas WHERE id_cuenta= cuenta.id_cuenta) order by  id_cuenta asc")->fetchAll(PDO::FETCH_OBJ);
