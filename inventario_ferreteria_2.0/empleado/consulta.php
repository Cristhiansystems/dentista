...
include ("../conexion.php");
    ...
    if($estado=="activo"){
	   $registros=$base->query("SELECT SQL_CALC_FOUND_ROWS  emp.id_empleado,concat_ws(' ',emp.nombre,emp.apellido_paterno,emp.apellido_materno) as empleado,concat_ws(' ',emp.ci,emp.extension) as ci , emp.celular, emp.fecha_nacimiento ,emp.login ,emp.foto FROM tbl_empleados emp where estado='$estado' order by apellido_paterno, apellido_materno, nombre limit $inicio , $regpagina ")->fetchAll(PDO::FETCH_OBJ);

    }else if($estado=="desactivo"){
        $registros=$base->query("SELECT SQL_CALC_FOUND_ROWS emp.id_empleado,concat_ws(' ',emp.nombre,emp.apellido_paterno,emp.apellido_materno) as empleado, concat_ws(' ',emp.ci,emp.extension) as ci , emp.celular, emp.fecha_nacimiento ,emp.login ,emp.foto FROM tbl_empleados emp where emp.estado='$estado' order by emp.apellido_paterno, emp.apellido_materno, emp.nombre limit $inicio , $regpagina ")->fetchAll(PDO::FETCH_OBJ);
    }
...
foreach($registros as $empleados):
    ...
    $html.="<td><a href='actualizar.php?id=$empleados->id_empleado'><input class='btn btn-success' type='button' name='ac' id='ac' value='actualizar'></a></td></tr>";
